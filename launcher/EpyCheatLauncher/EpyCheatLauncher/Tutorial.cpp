...
Tutorial::Tutorial(std::shared_ptr<sf::RenderWindow>& window, const std::vector<std::string>& lang) : _window(window)
{
    ...
    this->_listSprite["Tutorial"] = std::make_unique<Sprites>(".\\Ressources\\tuto_butt\\tuto_butt3.png", sf::Vector2f(86, 451));
    ...
    this->_currentTutorial = TUTO_BASE;
    this->_window = _window;
}

void Tutorial::update(sf::Clock& cl, const sf::Time& time, Scenes& currentScene)
{
    (void)time;
    (void)cl;
    (void)currentScene;
    ...
    this->_listSprite["Account"] = std::make_unique<Sprites>(".\\Ressources\\account_butt\\account_butt1.png", sf::Vector2f(86, 814));
    this->_listSprite["ScrollBar"] = std::make_unique<Sprites>(".\\Ressources\\scroll_bar\\scroll_bar.png", sf::Vector2f(1543, 43));
    if (this->_currentTutorial == TUTO_HOME) {
...
void Tutorial::MouseMove_Tutorial(void)
{
    ...
    for (i; str[i][0] != '\0'; i++) {
        ...
        if (frect.intersects(this->MouseRect) == true) {
            this->_currentTutorial = Tutorials(i);
            break;
        }
    }
    return;
...
void Tutorial::MouseClick_Tutorial(Scenes& currentScene)
{
    char str[][10] = { ("Home"), ("Shop"), ("Library"), ("Tutorial"), ("Downloads"), ("Settings"), ("Account"), ("ScrollBar"), ("\0") };
    int i = 0;
    sf::FloatRect frect = sf::FloatRect(0, 0, 0, 0);
...
void Tutorial::handleEvent(const sf::Event& event, Scenes& currentScene, sf::Clock& cl, const sf::Time& time)
{
    (void)time;
    (void)cl;
    ...
    if (event.type == sf::Event::MouseButtonPressed) {
        if (event.mouseButton.button == sf::Mouse::Left) {
            MouseClick_Tutorial(currentScene);
            std::cerr << "TEST TUTORIAL" << std::endl;
        }
    }
}
